...
1. **중복 청구** — duplicate_groups(코드가 금액·날짜·가맹점명을 결정론적으로
   대조해 이미 확신한 클러스터)에 있는 claim_id들만 중복으로 서술합니다. 이
   목록에 없는 조합을 스스로 중복이라고 판단하지 않습니다.
2. **미래 거래일** — check_future_dated_claims 툴을 후보 claim 전체에 대해 반드시
...
3. **애매한 패턴** — 위 두 유형에 안 걸리지만 의심스러운 조합은 당신의 판단으로
