...
def test_calls_within_limit_are_allowed_and_audited(audit_log):
    before = callbacks.make_before_tool_callback("EXECUTOR")
    tool, ctx = FakeTool("submit_settlement_analysis"), FakeToolContext()

    result = before(tool, {}, ctx)

    assert result is None  # None이면 ADK가 실제 툴을 호출한다
    assert audit_log == [
        {"agent": "EXECUTOR", "action": "TOOL_CALL_STARTED", "reason": "tool=submit_settlement_analysis"}
    ]
...
def test_call_count_persists_across_invocations_in_same_session(audit_log):
    ...
    assert counts["submit_settlement_analysis"] == 2
